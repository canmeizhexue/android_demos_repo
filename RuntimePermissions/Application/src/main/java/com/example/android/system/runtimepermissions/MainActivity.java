...
public class MainActivity extends SampleActivityBase {
    ...
    public void showCamera(View view) {
        ...
        if (PermissionUtil.hasSelfPermission(this, Manifest.permission.CAMERA)) {
            ...
            //是否需要提供一个合理的ui来解释下为什么需要相应的权限
            //1.第一次调用，返回false,然后点击禁止，不勾选不再询问，那么再次调用的时候，返回了true,如果一直点击禁止并且不勾选不再询问，那么就会一直返回true
            //2.如果拒绝权限，并且勾选不再询问，那么下次再调用的时候就会返回false,并且requestPermissions很快就返回了，并且不会再出现系统的权限对话框了
            //3.相应的权限还是需要在xml里面声明的，
            //4.如果app的target低于23，但是用户的手机是6.0+，那么还是可以撤销权限的，
